...
if ( ! defined( 'MAIL_BANK_VERSION_NUMBER' ) ) {
	define( 'MAIL_BANK_VERSION_NUMBER', '3.0.36' );
}
...
if ( $version >= '3.0.3' ) {
	...
	if ( is_admin() ) {
		...
		function backend_js_css_for_mail_bank() {
			...
			if ( in_array( isset( $_REQUEST['page'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) : '', $pages_mail_bank ) ) {
...
function mail_bank_admin_notice_class() {
	...
	class Mail_Bank_Admin_Notices {
		...
		public function mb_admin_notice_ignore() {
			...
			if ( isset( $_REQUEST['mb_admin_notice_ignore'] ) ) {
				$admin_notices_option = get_option( 'mb_admin_notice', array() );
				$admin_notices_option[ $_REQUEST['mb_admin_notice_ignore'] ]['dismissed'] = 1;
				update_option( 'mb_admin_notice', $admin_notices_option );
		...
		public function mb_admin_notice_temp_ignore() {
			...
			if ( isset( $_REQUEST['mb_admin_notice_temp_ignore'] ) ) {
				$admin_notices_option = get_option( 'mb_admin_notice', array() );
				$current_date         = current_time( 'm/d/Y' );
				$date_array           = explode( '/', $current_date );
				$interval             = ( isset( $_REQUEST['mb_int'] ) ? $_REQUEST['mb_int'] : 7 );

				$date_array[1] += $interval;
				$new_start      = date( 'm/d/Y', mktime( 0, 0, 0, $date_array[0], $date_array[1], $date_array[2] ) );

				$admin_notices_option[ $_REQUEST['mb_admin_notice_temp_ignore'] ]['start']     = $new_start;
				$admin_notices_option[ $_REQUEST['mb_admin_notice_temp_ignore'] ]['dismissed'] = 0;
				update_option( 'mb_admin_notice', $admin_notices_option );
		...
		public function mb_admin_notice_pages( $pages ) {
			foreach ( $pages as $key => $page ) {
				if ( is_array( $page ) ) {
					if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] === $page[0] && isset( $_REQUEST['tab'] ) && $_REQUEST['tab'] === $page[1] ) {
						return true;
					}
				} else {
					if ( 'all' === $page ) {
		...
		public function mb_display_admin_notices() {
			...
			$notices['two_week_review'] = array(
				'title'      => 'Leave A Review For Mail Bank ?',
...
function add_popup_on_deactivation_mail_bank() {
	 ...
	 */
	class Mail_Bank_Deactivation_Form {
		/**
		 * Initialize the class and set its properties.
		 */
		...
		function post_user_feedback_mail_bank() {
			$mail_bank_deactivation_reason = sanitize_text_field( wp_unslash( $_REQUEST['reason'] ) );
		...
		function add_deactivation_dialog_form_mail_bank() {
			...
			<p style="margin-top:-5px">We feel guilty when anyone stop using Mail Bank.</p>
						<p>If Mail Bank isn't working for you, others also may not.</p>
					...
					<li class="mail-bank-reason mail-bank-custom-input">
						<label>
							<span><input value="3" type="radio" name="reason" /></span>
														<span>Open a <a href="https://wordpress.org/support/plugin/wp-mail-bank" target="_blank">Support Ticket</a> for me.</span>
						</label>
					</li>
				</ul>
			</form>
