...
function _section_header($header, $sm = false, $ksesbrstrong = false) {
  ...
  $text = $ksesbrstrong ? wp_kses( $header, array( 'br' => array(), 'strong' => array(), 'span' => array( 'style' => array() ) ) ) : esc_html( $header );
...
function _institutions_by_service_group($group_id) {
  ...
  $clinics = _run_wp_query( 'clinic', 'clinics_by_sg', -1, array(), $tax_query, true);

  if( $clinics->have_posts()) {
    while( $clinics->have_posts()) { $clinics->the_post();
      $clinics_ids[] = get_the_ID();
    }
  }; wp_reset_postdata();

  if( $clinics_ids ) {
    foreach($clinics_ids as $id) {
      $meta_query[] = array(
  ...
  $institutions = _run_wp_query( 'institution', 'inst_by_sg', -1, $meta_query, array(), true);
...
function _get_pricelist_data() {
  ...
  $city = get_search_form_param('pcity');
    ...
    if( $groups ) {
      foreach($groups as $group) {
        ...
        $services = new WP_Query( array(
          ...
          'meta_query'  => array(
            array(
              'key'     => '_service_pricelist_%__price_'.absint( $city ),
              'value'   => '',
              'type'    => 'CHAR',
              'compare' => '!='
            )
          ...
          while( $services->have_posts() ) { $services->the_post();
            ...
            if( $pricelist ) {
              foreach($pricelist as $item) {
                $name   = $item['_name'];
                $price  = $item['_price_'.absint( $city )];
                $service_pricelist[] = array(
                  'name' => esc_html( $name ),
...
/**
 * This function will return array of cities with prices for given service
 * @param $service_id (int)
 * @return array
*/
function _service_pricelists($service_id) {
  ...
  if( $cities ) {
    foreach($cities as $city) {
      $pricelists = new WP_Query( array(
         ...
         'meta_query'  => array(
            array(
              'key'     => '_service_pricelist_%__price_'.absint( $city->term_id ),
              'value'   => '',
              'type'    => 'CHAR',
              'compare' => '!='
            )
      ...
      if( $pricelists->have_posts() ) {
        while( $pricelists->have_posts()) { $pricelists->the_post();
          $cities_prices[] = array(
            'id' => $city->term_id,
            'name' => $city->name
          );
...
function filter_wpcf7_form_tag( $tag, $replace ) {
  switch($tag['name']) {
    ...
    case 'your-institution':
      $institutions = _run_wp_query('institution', 'wpcf7', -1, array(), array(), false, 1, array(), 'name', 'ASC');

      $tag['raw_values'][] = pll_trans('Wybierz centrum medyczne', true);  
      $tag['values'][] = pll_trans('Wybierz centrum medyczne', true);  
      $tag['labels'][] = pll_trans('Wybierz centrum medyczne', true); 
...
function _register_setting_fields() {
  ...
  add_settings_field(
    'career_blog_desc_id',
    '<label for="career_blog_desc_id">' . __( 'Blog description - Business' , 'rfswp' ) . '</label>',
