<?php  
$tpay_active      = get_field('_tpay_active', 'option');
$transfer_active  = get_field('_transfer_active', 'option');
$product_types_in_cart = CORTEN_SHOP::cart_product_types();
?>
<form class="form-horizontal" action="<?php the_permalink(); ?>" method="post" name="checkout-form" id="checkout-form">
  <div class="checkout-form row">
    ...
    <div class="row checkout-customer-type">
      <div class="form-group text-center">
        <label class="radio-inline">
          ...
          <span><?php pll_trans('Osoba fizyczna'); ?></span></label>
        <label class="radio-inline">
    ...
    <div class="row">
      ...
      <div class="col-sm-6 checkout-form-right">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <input type="text" class="form-control required" id="customer-email" name="customer-email" placeholder="<?php pll_trans('Podaj adres email'); ?>" value="<?php echo esc_html( CORTEN_SHOP::customer_data_item('email') ); ?>">
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-sm-6 checkout-form-left">
        ...
        <p class="form-group-header"><strong><?php pll_trans('Dane adresowe zamawiającego'); ?></strong></p>
    ...
    <?php _get_template_part( 'order', 'shop/checkout' ); ?>

    <div class="row checkout-submit text-right">
      <!--<a href="<?php //echo CORTEN_SHOP::shop_links('cart'); ?>" class="btn btn-info pull-left"><?php //pll_trans('Wróć'); ?></a>-->
      <?php if( absint( $tpay_active ) == 1 ) : ?>
        <button type="submit" class="btn btn-primary payment-btn" value="online"><?php pll_trans('Płacę online'); ?><?php echo CORTEN_SHOP::cart_loader(); ?></button>
      <?php endif; ?>
      <?php if( absint( $transfer_active ) == 1 ) : ?>
        <button type="submit" class="btn btn-primary payment-btn" value="transfer"><?php pll_trans('Płacę przelewem'); ?><?php echo CORTEN_SHOP::cart_loader(); ?></button>
      <?php endif; ?>
    </div>
